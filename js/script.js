...
async function start(){
    ...
    for (let k = 0; k < arrJugs.length; k++){
        ...
        if(k===0){
            jug.propiedades.push(p12);

            jug.propiedades.push(p15);
            p12.propietario = jug;
            p15.propietario = jug;
            jug.dinero = 620;
        }else if(k===1){
            ...
            p28.hoteles=1;
            p30.casas=4;
            jug.propiedades.push(p27);
            jug.propiedades.push(p28);
            jug.propiedades.push(p30);
            jug.gruposCasillas.push(grupo6);
            jug.dinero = 780;

            grupo6.propietario = jug;
    ...
    mostrarHotel("div-" + p27.id + "-" + 1);
    mostrarHotel("div-" + p28.id + "-" + 1);
    for(let i = 1; i <= 4; i++){
        mostrarCasa("div-" + p30.id + "-" + i);
    }
    jugadores.forEach((jugador, indice)=>{
        jugador.propiedades.forEach(propiedad=>{
            const prop = document.getElementById(propiedad.id);
            prop.classList.add("casillaJugador" + (indice + 1));
        })
    })
